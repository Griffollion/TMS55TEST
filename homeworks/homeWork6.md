### 
- В папке solutions создать папку homework6

- В папке homework6 создать файлик index.html
...
- Для текущей домашки добавил задания 35-41. Если понимаете, что можете их решить - дейлайте в первую очередь их. Если нет - сделайте хотя бы четыре задания из предыдущих нерешенных задач. + Если не понимаете что вообще происходит пересмотрите видосы по функциям и объектам и попробуйте сделать те задания которые предлагают там в конце

- Перед кодом задания должен стоять комментарий номера задачи которую вы решаете. Дедлайн 04.07.2021 22:00. Все работы которые придут позже буду проверять по мере возможностей и времени. В приоритете будут работы сданные в срок.
